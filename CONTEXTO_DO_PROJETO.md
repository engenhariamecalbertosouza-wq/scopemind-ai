...
- ⚠️ **CELULAR ESTÁ BLOQUEADO TEMPORARIAMENTE** (economia de IA/cota) — só desktop entra. Reativar quando virar o plano pago (ver seção 12).
...
1. **🎯 Radar de Oportunidades** (era "Agenda") — vitrine inteligente do dia: **Destaques do Dia** + **cards** por jogo
   com o melhor mercado/probabilidade/confiança/risco e "outras leituras"; botões "Ver detalhes"/"Analisar". Como
   reaproveita análises salvas (ZERO IA), jogos sem análise aparecem "Aguardando análise"; admin clica "Analisar" e o
   jogo vira card com oportunidades. Por baixo ainda traz a lista de jogos a começar (abas Hoje/Amanhã, pastas por
   campeonato, ⭐ Favoritos, 🔔 Alertas). **Auto-atualiza a cada 10 min.** **Só campeonatos de DESTAQUE** (ver seção 6).
   Linguagem SEMPRE de análise/probabilidade, NUNCA promessa. (Detalhe técnico na seção 10.)
...
- **Etapa 2 (no ar):** `radar.py` deriva oportunidades + `value_score` + ranking + destaques; endpoint **GET `/api/radar`**
  ...
  `carregarRadar/renderRadar/rdrCardDestaque/rdrCardJogo/abrirDetalhesRadar` (+ `RDR_BADGE`). CSS `.rdr-*`. Filtros antigos escondidos.
...
- **Próximas etapas:** 3/4 (agrupamento/seções), 5 (filtros por mercado/risco/confiança), 6 (detalhes por abas), depois o
  **auto-análise EM LOTE** (caro: roda 1× no servidor/cron, com teto `MAX_ANALISES_DIA` + cache; **só LIGAR no plano Pro**).
...
3. **Radar — etapas futuras:** 3/4 (agrupamento), 5 (filtros), 6 (detalhes por abas), e por fim o **auto-análise em lote**
   (caro — só no plano Pro, com teto + cache + lembrete de custo; rodar 1× no servidor, NÃO por cliente).
4. **Reativar o celular:** quando virar o plano pago, remover o bloco `if (EH_CELULAR)` no fim do `app.js` (e o overlay
   `#bloqueio-celular`). O layout responsivo + menu gaveta já estão prontos.
